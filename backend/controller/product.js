...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!user) {
                return res.status(400).json({
                    error: "Email does not exist in the users database",
                });
            }
...
router.put(
    ...
    async (req, res) => {
        ...
        if (!email || !productId || quantity === undefined) {
            return res.status(400).json({
                error:
                    "Email, productId, and quantity are required",
            });
        }
        ...
        try {
            ...
            res.status(200).json({
                message:
                    "Cart product quantity updated successfully",
                cart: user.cart,
            });
        } catch (err) {
            console.error("Server error:", err);
